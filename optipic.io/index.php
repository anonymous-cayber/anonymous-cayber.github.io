...
// Версия модуля
define("_OPTIPIC_MODULE_VERSION_", "1.30");
...
class OptiPicIndex {
    ...
    public function getIgnorelist() {
        ...
        $ignorePaths = array_merge(self::getInternalIgnorelist(), $ignorePaths);
        $ignorePaths = array_unique($ignorePaths);
...
if(isset($_REQUEST["cmd"]) && $_REQUEST["cmd"]=="getversion")
{
    $_optipicVersion = "";
...
if(!isset($optipicCfg["secretkey"]) || !isset($_REQUEST["secretkey"]) || !$optipicCfg["secretkey"] || !isset($_REQUEST["cmd"]))
{
    require_once './help.php';
    exit;
}
...
if($_REQUEST["cmd"]=="docompress")
{
    ...
    // this is the old way to get files list
    if(isset($_POST["files"])) {
        $listFiles = $_POST["files"];
    }
    
    // this is the new way to get files list (ModSecurity passed)
    if(isset($_POST["base64paths"])) {
        $listFiles = array();
        foreach($_POST["base64paths"] as $base64path) {
            $listFiles[] = base64_decode($base64path);
        }
    }
    
    // if files list is empty - return the error
    if(!isset($listFiles) || !$listFiles || !is_array($listFiles) || !count($listFiles)) {
        echo optipicGetSerializedGzip(array(
            "error" => "no_files_in_post", 
        ));
        exit;
    }
    ...
    foreach($listFiles as $file)
    {
        ...
        curl_setopt($ch, CURLOPT_URL, "https://api.optipic.io/api/compress");
